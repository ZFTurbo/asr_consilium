...
class Qwen3ASRConfig(PretrainedConfig):
    ...
    def get_text_config1(self):
        # Проверяем, существует ли thinker_config
        if hasattr(self, "thinker_config") and self.thinker_config is not None:
            return self.thinker_config.get_text_config()

        # Если его нет, скорее всего модель использует старый формат
        # и текстовый конфиг лежит прямо в корне
